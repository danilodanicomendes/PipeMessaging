...
DedicatedServerUser::DedicatedServerUser(Server * server, TCHAR login[TAMLOGIN], TCHAR password[TAMPASS], short int tipo,
										 HANDLE hPipe) : 
DedicatedServer(server, login, password, tipo, hPipe) {};


DWORD WINAPI DedicatedServerUser::threadMethod(LPVOID param) {
	BOOL ret = FALSE;
	TCHAR buf[256];
	DWORD n;
